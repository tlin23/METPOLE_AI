...
def test_backend_connectivity(base_url):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            console.print(
                f"[red]✗ Backend returned status code {response.status_code}[/red]"
            )
            return False
    ...
    endpoints = [
        {
            "path": "/api/ask",
            "method": "POST",
            "data": {"question": "What is Metropole?", "top_k": 3},
        },
        {
            "path": "/api/query",
            "method": "POST",
            "data": {"query": "building rules", "n_results": 3},
        },
    ]
        ...
        try:
            if endpoint["method"] == "GET":
                ...
                response = requests.post(
                    f"{base_url}{endpoint['path']}", json=endpoint["data"]
                )

            if response.status_code in [200, 201]:
                console.print(
                    f"[green]✓ {endpoint['path']} ({endpoint['method']}) - Success[/green]"
                )
                results.append(
                    {"endpoint": endpoint, "success": True, "response": response.json()}
                )
            else:
                console.print(
                    f"[red]✗ {endpoint['path']} ({endpoint['method']}) - Failed with status {response.status_code}[/red]"
                )
                results.append(
                    {
                        "endpoint": endpoint,
            ...
            console.print(
                f"[red]✗ {endpoint['path']} ({endpoint['method']}) - Error: {str(e)}[/red]"
            )
    ...
    try:
        with Progress() as progress:
            ...
            response = requests.post(
                ...
                json={"url": "https://www.metropoleballard.com/home"},
            )
            ...
            if response.status_code in [200, 201]:
                ...
                progress.update(task, completed=100)
                console.print(
                    f"[red]✗ Crawl request failed with status {response.status_code}[/red]"
                )
                return False
    except requests.exceptions.RequestException as e:
...
def test_ask_question(base_url):
    ...
    try:
        ...
        response = requests.post(
            f"{base_url}/api/ask", json={"question": question, "top_k": 5}
        )
        ...
        if response.status_code in [200, 201]:
            ...
            if data.get("success"):
                ...
                # Display the answer
                console.print(
                    Panel(data.get("answer", "No answer provided"), title="Answer")
                )
                ...
                console.print(
                    f"[red]✗ Question answering failed: {data.get('message')}[/red]"
                )
                return None
        else:
            console.print(
                f"[red]✗ Question request failed with status {response.status_code}[/red]"
            )
            return None
    except requests.exceptions.RequestException as e:
...
def test_feedback_logging(base_url, question_data):
    ...
    if not question_data:
        console.print(
            "[yellow]! Skipping feedback test as question data is not available[/yellow]"
        )
        return False
    ...
    try:
        feedback_data = {
            ...
            "chunk_ids": [
                ...
                for chunk in question_data.get("chunks", [])
        ...
        if response.status_code in [200, 201]:
            ...
            if data.get("success"):
                ...
                console.print(
                    f"[red]✗ Feedback logging failed: {data.get('message')}[/red]"
                )
                return False
        else:
            console.print(
                f"[red]✗ Feedback request failed with status {response.status_code}[/red]"
            )
            return False
    except requests.exceptions.RequestException as e:
...
def main():
    ...
    if not test_backend_connectivity(args.backend_url):
        console.print(
            "[bold red]Backend connectivity test failed. Aborting further tests.[/bold red]"
        )
        sys.exit(1)
    ...
    table.add_row(
        "API Endpoints",
        f"[{'green' if all(r['success'] for r in api_results) else 'red'}]{'✓ All Passed' if all(r['success'] for r in api_results) else '✗ Some Failed'}[/{'green' if all(r['success'] for r in api_results) else 'red'}]",
    )
    table.add_row(
        "Crawl Functionality",
        f"[{'green' if crawl_success else 'red'}]{'✓ Passed' if crawl_success else '✗ Failed'}[/{'green' if crawl_success else 'red'}]",
    )
    table.add_row(
        "Question Answering",
        f"[{'green' if question_data else 'red'}]{'✓ Passed' if question_data else '✗ Failed'}[/{'green' if question_data else 'red'}]",
    )
    table.add_row(
        "Feedback Logging",
        f"[{'green' if feedback_success else 'red'}]{'✓ Passed' if feedback_success else '✗ Failed'}[/{'green' if feedback_success else 'red'}]",
    )
    ...
    if all_passed:
        console.print(
            Panel(
                "[bold green]All tests passed! The deployment is working correctly.[/bold green]",
                title="Success",
            )
        )
    else:
        console.print(
            Panel(
                "[bold yellow]Some tests failed. Please check the results above for details.[/bold yellow]",
                title="Warning",
            )
        )
