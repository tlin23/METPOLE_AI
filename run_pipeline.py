...
def crawl_website(start_url: str, max_pages: Optional[int] = None) -> Dict[str, str]:
    ...
    logger.info(f"Crawling complete. Collected {len(content_dict)} pages in {elapsed_time:.2f} seconds")
    return content_dict
...
def process_html_content() -> None:
    ...
    logger.info(f"Extracted {len(unique_chunks)} unique content chunks from {len(results)} HTML files")
...
def add_metadata_and_tags() -> None:
    ...
    logger.info(f"Processed {len(processed_objects)} content chunks from {page_count} unique pages")
    logger.info(f"Generated {tag_count} tags (avg. {tag_count/len(processed_objects):.1f} per chunk)")
