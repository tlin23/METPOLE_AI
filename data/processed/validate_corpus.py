...
def validate_corpus(
    corpus_path: str, verbose: bool = False
    ...
    for i, chunk in enumerate(corpus):
        ...
        if missing_fields:
            ...
            if verbose:
                print(
                    f"Warning: Chunk at index {i} is missing fields: {missing_fields}"
                )
        ...
        if not chunk_id.startswith("chunk_"):
            ...
            if verbose:
                print(
                    f"Warning: Chunk ID at index {i} should start with 'chunk_': {chunk_id}"
                )
        ...
        if not page_id.startswith("page_"):
            ...
            if verbose:
                print(
                    f"Warning: Page ID at index {i} should start with 'page_': {page_id}"
                )
        ...
        if page_name in page_name_to_id:
            if page_name_to_id[page_name] != page_id:
                ...
                if verbose:
                    print(
                        f"Warning: Inconsistent page_id for page '{page_name}': "
    ...
    validation_passed = (
        stats["chunks_with_missing_fields"] == 0
        and stats["duplicate_chunk_ids"] == 0
        and stats["invalid_chunk_id_format"] == 0
        and stats["invalid_page_id_format"] == 0
        and stats["inconsistent_page_ids"] == 0
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate the metropole_corpus.json file."
    )
    parser.add_argument(
        "--corpus-path",
        type=str,
